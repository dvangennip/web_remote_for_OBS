/**
 * This code is meant to run on a Palm Pre3 webOS phone, released in 2011.
 * Many modern javascript conventions won't work (for example, no let or const, no classes).
 * Some functionality can be polyfilled, others are out of scope.
 */
...
var MinimalRequest = function (url, data, callback) {
...
var OBSRemote = function () {
	...
	// enable exceptions for older browsers
	this.advanced_capabilities = true;
	if (navigator.userAgent.indexOf('webOS') != -1) {
		this.advanced_capabilities = false;
	}
...
OBSRemote.prototype = {
	...
	get_program_scene: function (ignoreUpdateScenes) {
	...
	get_preview_scene: function (ignoreUpdateScenes) {
	...
	set_scene: function (inSceneName) {
	...
	set_source_screenshot: function (isPreview) {
		...
		window.setTimeout(function () {
			for (var i = 0; i < this.audio_list.length; i++) {
				...
				if (!li_el) {
					...
					// make exception for browser without input[type=range] support
					// TODO make this universal
					if (!this.advanced_capabilities) {
						var min_el = Element.make('button', {
							'id'       : 'audio_' + slug + '_min_button',
							'className': 'audio_volume_button',
							'innerHTML': '-',
							'events'   : {
								'click': this.set_volume_delta.bind(this, name, 'min')
							}
						});
						var plus_el = Element.make('button', {
							'id'       : 'audio_' + slug + '_max_button',
							'className': 'audio_volume_button',
							'innerHTML': '+',
							'events'   : {
								'click': this.set_volume_delta.bind(this, name, 'plus')
							}
						});

						li_el.appendChild(min_el);
						li_el.appendChild(plus_el);
					} else {
					...
					var label_el = document.getElementById('label_' + slug),
	...
	set_volume: function (inSource, inVolume) {
		var vol = inVolume;
	...
	set_volume_delta: function (inSource, inDirection) {
		for (var i = 0; i < this.audio_list.length; i++) {
			if (this.audio_list[i].name == inSource) {
				var v = this.audio_list[i]['volume'];
				...
				this.set_volume(inSource, v);
			}
		}
	},

	toggle_mute: function (inSource) {
	...
	create_audio_output: function () {
		// TODO just a test at the moment

		// create audio element
		var audio_player = Element.make('audio', {
			'id': 'audio_player',
			'controls': 'controls'
		});
		document.getElementsByTagName('body')[0].appendChild(audio_player);

		// connect to mic and monitor it
		navigator.mediaDevices.getUserMedia({ audio: true, video: false })
			.then(function (stream) {
				var audio_player = document.getElementById('audio_player');

				if (window.URL) {
					audio_player.srcObject = stream;
				} else {
	...
	update_status_list: function () {
		...
		new MinimalRequest('/call/GetStats', {}, function (response) {
			if (response.status == 'ok') {
				...
				var is_alert_state = (response.stats['output-skipped-frames'] / response.stats['output-total-frames'] > 0.05);
				if (!is_alert_state) {
			...
			if (response.status == 'ok') {
				...
				for (var i = 0; i < response.filters.length; i++) {
					...
					if (create_filter) {
						...
						var filter_instance = new SourceFilter(inSource, filter.name, filter.type, filter.enabled, settings);
	...
	handle_key_event: function (inEvent) {
...
var SourceFilter = function (inSource, inName, inType, inEnabled, inSettings) {
...
SourceFilter.prototype = {
	...
	get_filter_settings: function (inSource, inFilter) {
	...
	set_filter_setting: function (inSetting, inValue) {
		var key_value_pair = {};
